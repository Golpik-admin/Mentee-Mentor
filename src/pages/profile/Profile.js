...
const Profile = () => {
  ...
  const handleSendRequest = () => {
    // Simulate sending a request (you can replace this with an API call)
    // After the request is sent, update the state
    setRequestSent(true);
    // Perform any additional actions (e.g., API request)
  };

  const handleCancelRequest = () => {
    // Simulate canceling a request (you can replace this with an API call)
    setRequestSent(false);
    // Perform any additional actions (e.g., API request to cancel)
  };
        ...
        {userType === "mentee" ? (
          <div className="flex items-center gap-4">
            ...
            <button
              ...
              type="submit"
              className={` ${
                requestSent ? "bg-[#DF4C4C]" : "bg-colorPrimary"
              } text-colorWhite text-sm flex justify-center font-semibold flex flex-row cursor-pointer gap-4 items-center  px-4 py-2 rounded-lg`}
            >
              <PancilSvg />
              {requestSent ? "Cancel Request" : "Send Request"}
            </button>
          </div>
        ) : (
          <div>
